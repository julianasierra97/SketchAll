...
public class EditeurClient extends JPanel {
	...
	public EditeurClient (FrameClient frame) {
		...
		try {
			...
			for (RemoteDessinServeur rd : dessinsDistants) {
				addDrawing (rd, rd.getName(), rd.getX(), rd.getY(), rd.getWidth(), rd.getHeight(), rd.getColor(), rd.getShapeType()) ;
			}
		} catch (Exception e) {
	...
	public void addDrawing (RemoteDessinServeur proxy, String proxyName, int x, int y, int w, int h, Color color, String shapeType) {
